...
public class ThreadPoolFactoryUtils {
    ...
    public static ExecutorService createCustomThreadPoolIfAbsent(CustomThreadPoolConfig customThreadPoolConfig,
        ...
        ExecutorService threadPool = THREAD_POOLS.computeIfAbsent(
                threadNamePrefix,
                k -> createThreadPool(customThreadPoolConfig, threadNamePrefix, dameon)
        );
        if (threadPool.isShutdown() || threadPool.isTerminated()) {
            THREAD_POOLS.remove(threadNamePrefix);
            threadPool = createThreadPool(customThreadPoolConfig, threadNamePrefix, dameon);
    ...
    private static ExecutorService createThreadPool(CustomThreadPoolConfig customThreadPoolConfig,
                                                    String threadNmePrefix,
                                                    Boolean daemon) {
        ThreadFactory threadFactory = createThreadFactory(threadNmePrefix, daemon);
    ...
    public static void printThreadPoolStatus(ThreadPoolExecutor threadPool) {
        ...
        scheduledExecutorService.scheduleAtFixedRate(() -> {
            ...
            log.info("Number of Tasks of Completed: [{}]", threadPool.getCompletedTaskCount());
